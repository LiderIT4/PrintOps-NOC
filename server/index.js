...
function log(msg) {
  ...
  console.log(msg); // minimal console output
  fs.appendFileSync(path.join(__dirname, 'server.log'), line);
}

log("Server starting...");

// Initialize printers with user provided data
let printers = {
  "Pastor": {
...
const CACHE_duration = 3600000; // 1 hour for full refresh

// Scrapers
const toshibaScraper = require('./scrapers/toshiba');
const hpScraper = require('./scrapers/hp');
...
app.get('/', (req, res) => {
  res.send('PrintOps NOC Backend is running. Access API at /api/printers');
});

// Queue system to limit concurrency
const MAX_CONCURRENT_SCRAPES = 2; // Limit to 2 browsers at once to save resources

async function scrapePrinter(printerName) {
  const printer = printers[printerName];
  if (!printer) return;

  log(`Starting scrape for ${printerName} (${printer.ip})...`);
  let browser;
  try {
    ...
    let inkLevels = [];
    if (printer.name.toLowerCase().includes('toshiba')) {
      inkLevels = await toshibaScraper.scrape(page, printer);
    } else {
      inkLevels = await hpScraper.scrape(page, printer);
    }

    // Update printer data
    printers[printerName] = {
      ...printer,
      inkLevels: inkLevels.length > 0 ? inkLevels : printer.inkLevels, // Keep old if fail
      timestamp: new Date().toISOString(),
      status: inkLevels.length > 0 ? 'online' : 'warning' // warning if scrape returned empty but didn't throw
    };
    log(`SUCCESS: Scraped ${printerName}`);

  } catch (error) {
    log(`FAILED: Scraped ${printerName} - ${error.message}`);
    printers[printerName] = {
      ...printer,
      status: 'offline' // Assume offline if connection failed
    };
  } finally {
...
async function scrapeAllPrinters() {
  log("Starting full network scan...");
  const printerNames = Object.keys(printers);

  // Process in chunks
  for (let i = 0; i < printerNames.length; i += MAX_CONCURRENT_SCRAPES) {
    const chunk = printerNames.slice(i, i + MAX_CONCURRENT_SCRAPES);
    await Promise.all(chunk.map(name => scrapePrinter(name)));
  }
  log("Full network scan complete.");
}
...
app.post('/api/refresh', async (req, res) => {
  ...
  // Trigger individual scrape (async to not block response)
  scrapePrinter(printerName);
...
app.listen(PORT, () => {
  ...
  setTimeout(scrapeAllPrinters, 5000);

  // Schedule periodic scans (every 1 hour)
  setInterval(scrapeAllPrinters, 60 * 60 * 1000);
});
