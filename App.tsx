...
import { subscribeToPrinterStatus } from './services/printerService';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToPrinterStatus(
      (updatedPrinter) => {
        setPrinters((prev) => {
          const exists = prev.find(p => p.name === updatedPrinter.name);
          if (exists) {
